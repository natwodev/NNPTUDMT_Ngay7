...
router.post('/excel_users', uploadExcel.single('file'), async function (req, res, next) {
    if (!req.file) {
        ...
        let role = await rolesModel.findOne({ name: 'USER' })
        if (!role) {
            fs.unlinkSync(pathFile)
        ...
        for (let index = 2; index <= worksheet.rowCount; index++) {
            ...
            let emailRaw = element.getCell(2).value;
            let email = typeof emailRaw === 'object' ? emailRaw.text : emailRaw;
            let password = crypto.randomBytes(8).toString('hex');

            try {
                await userController.CreateAnUser(
                    username, password, email, role._id
                )
